...
- "Чому ти не сказав про X?" / "Я задала питання" / "ти не відповів" / "уточни" /
  "перепитую" / "повтори краще" / "не повністю" → correction (skarga that bot's PREVIOUS
   answer was incomplete/wrong — НЕ evaluate_my_answer!)
  ⚠️ Це НЕ оцінка роботи менеджера — це скарга на твою попередню відповідь.
  Маршрутизація → bot_retry: переосмислити попередню відповідь з ширшим контекстом.
...
**КОСМЕЦЕВТИКА — запити про засоби догляду:**
- "Який засіб ESSE для вмивання" / "крем для рук Esse" / "сироватка Esse при куперозі" → info_indications (показання)
- "Засіб для вмивання вікової шкіри" БЕЗ бренду → out_of_scope (можливо)
- "Маска ESSE для проблемної шкіри" → info_indications
- "Лінія Esse Sensitive для атопії" → info_indications
- ⛔ НЕ відносити до operational/regulations навіть якщо звучить «побутово»: «засіб для X»,
  «крем для Y», «маска для Z» з явним брендом ESSE/Esse → це **продуктова рекомендація**.
