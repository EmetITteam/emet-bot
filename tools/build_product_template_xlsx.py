...
JSON_INPUT = PROJECT_ROOT / "data" / "products_template_v2.json"
...
COLUMNS = [
    ...
    ("shelf_life", "Термін придатності", 18, "med"),
...
def add_instruction_sheet(wb: Workbook):
    ...
    instructions = [
        ...
        (
            ...
            "відповіді стануть точнішими (~85% → ~95% якості).\n\n"
            "💡 У файлі НЕ всі продукти — ESSE-лінія обробляється окремо великою таблицею.",
            11, False, None
        ),
        ("", 11, False, None),
        ("👥 Хто заповнює (за кольором заголовків таблиці)", 14, True, "066AAB"),
        (
            "🟦 СИНІ заголовки = МЕДВІДДІЛ — об'єктивні факти (~31 поле):\n"
            "    склад, показання, протипоказання, протокол, зберігання, сертифікація, механізм дії.\n\n"
            "🟥 ЧЕРВОНІ заголовки = SALES DIRECTOR — sales playbook (8 полів):\n"
            "    USP, порівняння з конкурентами, заперечення, killer phrase, cross-sell, ціновий сегмент,\n"
            "    профіль пацієнта, посилання на До/Після (попередньо заповнено з бази бота).",
            11, False, None
        ),
        ("", 11, False, None),
        ("⏱ Скільки часу займе", 14, True, "066AAB"),
        (
            "• МЕДВІДДІЛ: ~25-30 хв на продукт × 16 продуктів = ~7-8 годин (1 тиждень розтягнуто).\n"
            "  Багато полів вже попередньо заповнено — треба перевірити і доповнити.\n"
            "• SALES DIRECTOR: ~30-40 хв на продукт × 16 = ~10-12 годин (3 робочі дні).\n"
            "  Sales-поля треба ПЕРЕПИСАТИ майже повністю — попередні значення часто маркетингова вода.",
            11, False, None
        ),
        ("", 11, False, None),
        ("🎨 Кольори клітинок (зміст)", 14, True, "066AAB"),
        (
            "🟢 ЗЕЛЕНИЙ — попередньо заповнено з бази бота. ПЕРЕВІРИТИ і виправити якщо неточно.\n"
            ...
            "📝 Якщо інформації нема — лиши порожнім (краще пусте ніж вигадане).",
            11, False, None
        ),
        ("", 11, False, None),
        ...
        (
            ...
            "6. ВАРІАНТИ ПРОДУКТІВ — кожен у своєму рядку (Ellansé S = окремо, M = окремо).\n"
            "7. ЦІНИ І МАРЖА — НЕ ВКАЗУВАТИ. У нас різні пропозиції під типи клієнтів.\n"
            "   Замість — поле «Ціновий сегмент»: бюджет / середній / преміум / люкс.",
            11, False, None
        ),
        ("", 11, False, None),
        ...
        ("", 11, False, None),
        ("📞 Питання? / Як виправити?", 14, True, "066AAB"),
        (
            "• Якщо незрозуміло поле — дивись лист «📚 Глосарій» (3-й вгорі).\n"
            "• Приклад заповненого продукту-еталону — docs/product_template_filled_examples.md (Ellansé S).\n"
            "• Якщо в зеленій клітинці помилка — просто переписуєш текст. Якщо треба видалити — Delete.\n"
            "• Питання — IT Director.",
            11, False, None
        ),
        ("", 11, False, None),
        ("🚀 Що буде після заповнення", 14, True, "066AAB"),
        (
            "1. Файл повертається IT Director (через GitHub або email).\n"
            "2. Скрипт автоматично конвертує Excel → формат для бота (без ручної роботи).\n"
            "3. Бот переіндексує базу знань — нові дані доступні через 5 хвилин.\n"
            "4. Очікуваний результат: точність відповідей бота 85% → 95%.\n"
            "5. Новий quality report через 24 год покаже покращення.",
            11, False, None
        ),
    ]
